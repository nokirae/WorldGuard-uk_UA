...
public class DriverMigration extends AbstractMigration {
    ...
    protected void migrate(RegionDatabase store) throws MigrationException {
        ...
        log.info("Завантаження регіонів для '" + store.getName() + "' зі старим драйвером...");
    ...
    private void write(String name, Set<ProtectedRegion> regions) throws MigrationException {
        log.info("Збереження даних для '" + name + "' з новим драйвером...");
        ...
        try {
            ...
            throw new MigrationException("Не вдалося зберегти дані регіону для '" + store.getName() + "' до нового драйвера", e);
        }
    }
