...
public class SimpleFlagRegistry implements FlagRegistry {
    ...
    public void register(Flag<?> flag) throws FlagConflictException {
        synchronized (lock) {
            if (initialized) {
                throw new IllegalStateException("На даний момент нові прапори не можуть бути зареєстровані.");
            }
    ...
    private Flag<?> forceRegister(Flag<?> flag) throws FlagConflictException {
        ...
        synchronized (lock) {
            ...
            if (flags.containsKey(name)) {
                throw new FlagConflictException("Прапор вже існує під назвою " + name);
            }
            ...
            if (flag != null) {
                try {
                    ...
                    if (unmarshalled != null) {
                        ...
                        log.warning("Не вдалося проаналізувати прапор '" + flag.getName() + "' зі значенням '" + entry.getValue() + "'");
                    }
                } catch (Throwable e) {
                    log.log(Level.WARNING, "Не вдалося розібрати значення прапора для " + flag, e);
                }
            }
        }
