    ...
    public DefaultDomain call() throws UnresolvedNamesException {
        ...
        if (!namesToQuery.isEmpty()) {
            try {
                ...
                throw new UnresolvedNamesException("Пошукова служба UUID не спрацювала, тому введені імена не могли бути перетворені в UUID");
            } catch (InterruptedException e) {
                throw new UnresolvedNamesException("Пошук UUID був перерваний");
            }
        }

        if (!namesToQuery.isEmpty()) {
            throw new UnresolvedNamesException("Неможливо визначити імена " + Joiner.on(", ").join(namesToQuery));
        }
