...
public class WorldGuardCommands {
    ...
    @Command(aliases = {"version"}, desc = "Отримати версію WorldGuard", max = 0)
    ...
    public void reload(CommandContext args, Actor sender) throws CommandException {
        ...
        if (!tasks.isEmpty()) {
            throw new CommandException("Наразі є невиконані завдання. Використовуйте /wg running для контролю цих завдань спочатку.");
        }
        ...
        try {
            ...
            // WGBukkit.cleanCache();
            sender.print("WorldGuard перезавантаження конфігурації.");
        } catch (Throwable t) {
            sender.printError("Помилка під час перезавантаження: " + t.getMessage());
        } finally {
    ...
    @Command(aliases = {"report"}, desc = "Написання звіту про WorldGuard", flags = "p", max = 0)
    ...
    public void report(CommandContext args, final Actor sender) throws CommandException, AuthorizationException {
        ...
        try {
            ...
            sender.print("WorldGuard звіт, написаний до " + dest.getAbsolutePath());
        } catch (IOException e) {
            throw new CommandException("Не вдалося написати звіт: " + e.getMessage());
        }
        
        if (args.hasFlag('p')) {
            sender.checkPermission("worldguard.report.pastebin");
            ActorCallbackPaste.pastebin(worldGuard.getSupervisor(), sender, result, "WorldGuard звіт: %s.report");
        }
    }

    @Command(aliases = {"profile"}, usage = "[-p] [-i <interval>] [-t <thread filter>] [<minutes>]",
            desc = "Профілювання використання процесора сервера", min = 0, max = 1,
            flags = "t:i:p")
    ...
    public void profile(final CommandContext args, final Actor sender) throws CommandException, AuthorizationException {
        ...
        if (args.argsLength() == 0) {
            ...
            if (minutes < 1) {
                throw new CommandException("Ви повинні запустити профілювання принаймні на 1 хвилину.");
            } else if (minutes > 10) {
                throw new CommandException("Ви можете створювати профілювання протягом максимум 10 хвилин.");
            }
        }
        ...
        if (args.hasFlag('i')) {
            ...
            if (interval < 1 || interval > 100) {
                throw new CommandException("Інтервал повинен бути від 1 до 100 (в мілісекундах)");
            }
            if (interval < 10) {
                sender.printDebug("Примітка: Низький інтервал може спричинити додаткове уповільнення під час профілювання.");
            }
        }
        Sampler sampler;
        ...
        synchronized (this) {
            if (activeSampler != null) {
                throw new CommandException("Профілювання наразі знаходиться в процесі створення! Будь ласка, скористайтеся /wg stopprofile щоб скасувати поточне профілювання.");
            }
        ...
        sender.print(TextComponent.of("Початок профілювання процесора. Результати будуть доступні через " + minutes + " хвилин.", TextColor.LIGHT_PURPLE)
                .append(TextComponent.newline())
                .append(TextComponent.of("Використовуйте ", TextColor.GRAY))
                ...
                .append(TextComponent.of(" в будь-який час, щоб скасувати профілювання ЦП.", TextColor.GRAY)));

        worldGuard.getSupervisor().monitor(FutureForwardingTask.create(
                sampler.getFuture(), "Профілювання ЦП для " + minutes + " хвилин", sender));
        ...
        Futures.addCallback(sampler.getFuture(), new FutureCallback<>() {
            ...
            public void onSuccess(Sampler result) {
                ...
                try {
                    ...
                    sender.print("Дані профілювання ЦП, записані в " + dest.getAbsolutePath());
                } catch (IOException e) {
                    sender.printError("Не вдалося записати дані профілювання процесора: " + e.getMessage());
                }

                if (pastebin) {
                    ActorCallbackPaste.pastebin(worldGuard.getSupervisor(), sender, output, "Результат профілювання: %s.profile");
                }
            }
    ...
    @Command(aliases = {"stopprofile"}, usage = "",desc = "Зупинити запущену профілювання", min = 0, max = 0)
    @CommandPermissions("worldguard.profile")
    public void stopProfile(CommandContext args, final Actor sender) throws CommandException {
        synchronized (this) {
            if (activeSampler == null) {
                throw new CommandException("Наразі не працює жодне профілювання процесора.");
            }
        ...
        sender.print("Профілювання роботи процесора було скасовано.");
    }

    @Command(aliases = {"flushstates", "clearstates"},
            usage = "[player]", desc = "Очистити диспетчер стану", max = 1)
    ...
    public void flushStates(CommandContext args, Actor sender) throws CommandException {
        if (args.argsLength() == 0) {
            ...
            sender.print("Очищено всі стани.");
        } else {
            ...
            if (player != null) {
                ...
                sender.print("Очищено стани для особи \"" + player.getName() + "\".");
            }
        }
    }

    @Command(aliases = {"running", "queue"}, desc = "Список поточних завдань", max = 0)
    ...
    public void listRunningTasks(CommandContext args, Actor sender) throws CommandException {
        ...
        if (tasks.isEmpty()) {
            sender.print("Наразі немає активних завдань.");
        } else {
            tasks.sort(new TaskStateComparator());
            MessageBox builder = new MessageBox("Запущені завдання", new TextComponentProducer());
            builder.append(TextComponent.of("Примітка: деякі 'running' завдання можуть очікувати на запуск.", TextColor.GRAY));
            for (Task<?> task : tasks) {
    ...
    @Command(aliases = {"debug"}, desc = "Команди налагодження")
