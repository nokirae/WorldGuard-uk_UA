...
class FlagHelperBox extends PaginationBox {
    ...
    FlagHelperBox(World world, ProtectedRegion region, RegionPermissionModel perms) {
        super("Прапори для " + region.getId(), "/rg flags -w \"" + world.getName() + "\" -p %page% " + region.getId());
        this.world = world;
        this.region = region;
        this.perms = perms;
    }
    ...
    public Component getComponent(int number) {
        if (number == Flags.INBUILT_FLAGS.size()) {
            return centerAndBorder(TextComponent.of("Прапори третіх сторін", TextColor.AQUA));
        ...
        if (flag.usesMembershipAsDefault()) {
            builder.append(TextComponent.empty().append(TextComponent.of("*", TextColor.AQUA))
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of("Це спеціальний прапор, який за замовчуванням дає доступ для учасників та забороняє доступ для не учасників."))));
            length += monoSpace ? 1 : FlagFontInfo.getPxLength('*');
        }
        if (flag == Flags.PASSTHROUGH) {
            builder.append(TextComponent.empty().append(TextComponent.of("*", TextColor.AQUA))
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of("Це спеціальний прапор, який замінює перевірки будівлі. (Не пов'язаний з рухом!)"))));
            length += monoSpace ? 1 : FlagFontInfo.getPxLength('*');
        }
    ...
    private <V> void appendValueChoices(TextComponent.Builder builder, Flag<V> flag, Iterator<V> choices, @Nullable String suggestChoice) {
        ...
        while (choices.hasNext()) {
            ...
            if (maySet) {
                if (isExplicitSet) {
                    hoverTexts.add(TextComponent.of("Натисніть, щоб скинути", TextColor.GOLD));
                } else if (DANGER_ZONE.contains(flag) && !(ProtectedRegion.GLOBAL_REGION.equals(region.getId()) && flag == Flags.PASSTHROUGH)) {
                    hoverTexts.add(TextComponent.of("Встановлення цього прапорця може мати непередбачувані наслідки.", TextColor.RED)
                            .append(TextComponent.newline())
                            .append(TextComponent.of("Будь ласка, прочитайте документацію та встановіть цей прапор вручну, якщо ви дійсно маєте намір це зробити.")
                            .append(TextComponent.newline())
                            .append(TextComponent.of("(Підказка: Вам не потрібно встановлювати це, щоб захистити регіон!)"))));
                } else {
                    hoverTexts.add(TextComponent.of("Натисніть, щоб встановити", TextColor.GOLD));
                }
            }
        ...
        if (suggestChoice != null && perms.maySetFlag(region, flag)) {
            builder.append(TextComponent.of(suggestChoice, TextColor.DARK_GRAY)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of("Натисніть, щоб встановити власне значення", TextColor.GOLD)))
    ...
    private <V> void appendValueText(TextComponent.Builder builder, Flag<V> flag, String display, @Nullable Component hover) {
        ...
        if (maySet) {
            if (isExplicitSet) {
                hoverTexts.add(TextComponent.of("Натисніть, щоб змінити", TextColor.GOLD));
            } else {
                hoverTexts.add(TextComponent.of("Натисніть, щоб встановити", TextColor.GOLD));
            }
        }
    ...
    private <V> Component getToolTipHint(V defVal, V currVal, boolean inherited) {
        ...
        if (inherited) {
            if (currVal == defVal) {
                valType = TextComponent.of("Успадковане & ")
                        .append(TextComponent.of("за замовчуванням")
                                .decoration(TextDecoration.UNDERLINED, true))
                        .append(TextComponent.of(" значення"));
            } else {
                valType = TextComponent.of("Успадковане значення");
            }
        } else {
            if (currVal == defVal) {
    ...
    private <V> void appendSetFlagValue(TextComponent.Builder builder, SetFlag<V> flag) {
        ...
        if (currVal != null) {
            hoverComp = hoverComp.append(TextComponent.of("Поточні значення:"))
    ...
    private void appendLocationFlagValue(TextComponent.Builder builder, LocationFlag flag) {
        ...
        if (currVal == null) {
            ...
            if (defVal == null) {
                appendValueText(builder, flag, "невизначена локація", null);
            } else {
                appendValueText(builder, flag, defVal.toString(), TextComponent.of("За замовчуванням значення:")
                        .append(TextComponent.newline()).append(TextComponent.of(defVal.toString())));
            }
        } else {
            appendValueText(builder, flag, currVal.toString(), TextComponent.of("Поточне значення:")
    ...
    private <V extends Number> void appendNumericFlagValue(TextComponent.Builder builder, NumberFlag<V> flag) {
        ...
        appendValueChoices(builder, flag, (Iterator<V>) choices.iterator(), choices.isEmpty() ? "невизначена цифра" : "[власний]");
    }
    ...
    private void appendStringFlagValue(TextComponent.Builder builder, StringFlag flag) {
        ...
        if (currVal == null) {
            ...
            if (defVal == null) {
                appendValueText(builder, flag, "невизначений рядок", null);
            } else {
                ...
                appendValueText(builder, flag, display, TextComponent.of("За замовчуванням значення:")
            ...
            appendValueText(builder, flag, display, TextComponent.of("Поточне значення:")
