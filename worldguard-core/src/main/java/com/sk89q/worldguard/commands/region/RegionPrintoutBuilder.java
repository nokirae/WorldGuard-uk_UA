...
public class RegionPrintoutBuilder implements Callable<TextComponent> {
    ...
    public void appendBasics() {
        builder.append(TextComponent.of("Регіон: ", TextColor.BLUE));
        ...
        builder.append(TextComponent.of(", пріоритет=", TextColor.GRAY));
    ...
    public void appendFlags() {
        builder.append(TextComponent.of("Прапори: ", TextColor.BLUE));
    ...
    public void appendFlagsList(boolean useColors) {
        ...
        for (Flag<?> flag : WorldGuard.getInstance().getFlagRegistry()) {
            ...
            if (perms != null && perms.maySetFlag(region, flag)) {
                flagText = flagText.hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of("Натисніть, щоб встановити прапор")))
        ...
        if (!hasFlags) {
            TextComponent noFlags = TextComponent.of("(немає)", useColors ? TextColor.RED : TextColor.WHITE);
            builder.append(noFlags);
        }

        if (perms != null && perms.maySetFlag(region)) {
            builder.append(TextComponent.space())
                    .append(TextComponent.of("[Прапори]", useColors ? TextColor.GREEN : TextColor.GRAY)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of("Натисніть, щоб встановити прапор")))
        ...
        while (it.hasPrevious()) {
            ...
            if (perms != null && perms.mayLookup(cur)) {
                builder.append(TextComponent.of(cur.getId(), useColors ? TextColor.GREEN : TextColor.WHITE)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of("Натисніть для інформації")))
            ...
            if (!cur.equals(region)) {
                builder.append(TextComponent.of(" (батьківський, пріоритет=", useColors ? TextColor.GRAY : TextColor.WHITE));
            ...
            if (last != null && cur.equals(region) && perms != null && perms.maySetParent(cur, last)) {
                ...
                builder.append(TextComponent.of("[X]", TextColor.RED)
                        .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of("Натисніть, щоб від'єднати батьківський ")))
    ...
    public void appendDomain() {
        builder.append(TextComponent.of("Власники: ", TextColor.BLUE));
        ...
        builder.append(TextComponent.of("Учасники: ", TextColor.BLUE));
    ...
    private void addDomainString(DefaultDomain domain, String addCommand, String removeCommand) {
        if (domain.size() == 0) {
            builder.append(ErrorFormat.wrap("(немає)"));
        } else {
            if (perms != null) {
        ...
        if (addCommand != null) {
            builder.append(TextComponent.space().append(TextComponent.of("[Додати]", TextColor.GREEN)
                            .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of("Натисніть, щоб додати особу або групу")))
                            .clickEvent(ClickEvent.of(ClickEvent.Action.SUGGEST_COMMAND,
                                    "/rg " + addCommand + " -w \"" + world + "\" " + region.getId() + " "))));
        }
        if (removeCommand != null && domain.size() > 0) {
            builder.append(TextComponent.space().append(TextComponent.of("[Прибрати]", TextColor.RED)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of("Натисніть, щоб прибрати особу або групу")))
                    .clickEvent(ClickEvent.of(ClickEvent.Action.SUGGEST_COMMAND,
                            "/rg " + removeCommand + " -w \"" + world + "\" " + region.getId() + " "))));
            builder.append(TextComponent.space().append(TextComponent.of("[Очистити]", TextColor.RED)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of("Натисніть, щоб очистити")))
                    .clickEvent(ClickEvent.of(ClickEvent.Action.SUGGEST_COMMAND,
    ...
    public void appendBounds() {
        ...
        builder.append(TextComponent.of("Межі:", TextColor.BLUE));
        TextComponent bound = TextComponent.of(" " + min + " -> " + max, TextColor.YELLOW);
        if (perms != null && perms.maySelect(region)) {
            bound = bound
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of("Натисніть, щоб вибрати")))
        ...
        if (teleFlag != null && perms != null && perms.mayTeleportTo(region)) {
            builder.append(TextComponent.space().append(TextComponent.of("[Телепорт]", TextColor.GRAY)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of("Натисніть, щоб телепортуватися").append(TextComponent.newline()).append(
            ...
            builder.append(TextComponent.space().append(TextComponent.of("[Центр телепортації]", TextColor.GRAY)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of("Натисніть, щоб телепортуватися в центр регіону")))
                    .clickEvent(ClickEvent.of(ClickEvent.Action.RUN_COMMAND,
    ...
    private void appendPriorityComponent(ProtectedRegion rg) {
        ...
        if (perms != null && perms.maySetPriority(rg)) {
            builder.append(TextComponent.of(content, TextColor.GOLD)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of("Натисніть, щоб змінити")))
    ...
    private void appendRegionInformation() {
        ...
        if (cache != null && perms == null) {
            builder.append(SubtleFormat.wrap("Будь-які імена з суфіксом * є 'останніми відомими іменами' і можуть бути неактуальними."));
        }
    }

    @Override
    public TextComponent call() {
        MessageBox box = new MessageBox("Інформація про регіон", builder);
