...
public class BukkitDebugHandler implements DebugHandler {
    ...
    private <T extends Event & CancelLogging> void testEvent(CommandSender receiver, Player target, T event, boolean stacktraceMode) throws
        ...
        if (!receiver.equals(target)) {
            if (!isConsole) {
                log.info(receiver.getName() + " імітує подію на " + target.getName());
            }

            target.sendMessage(
                    ChatColor.RED + "(Будь ласка, ігноруйте будь-які повідомлення, які можуть з'явитися відразу після цього.)");
        }
        ...
        if (stacktraceMode) {
            receiver.sendMessage(ChatColor.GRAY + "Звіт був надрукований в консолі.");
            log.info("Звіт про подію для " + receiver.getName() + ":\n\n" + result);
            ...
            ActorCallbackPaste.pastebin(WorldGuard.getInstance().getSupervisor(), plugin.wrapCommandSender(receiver),
                    result, "Звіт про відлагодження події: %s.txt");
        } else {
            receiver.sendMessage(result.replaceAll("(?m)^", ChatColor.AQUA.toString()));

            if (result.length() >= 500 && !isConsole) {
                receiver.sendMessage(ChatColor.GRAY + "Звіт також був надрукований для консолі.");
        ...
        if (fromTarget) {
            ...
            if (sender instanceof Player) {
                ...
                throw new CommandException(
                        "Якщо ця команда не буде використовуватися в грі, використовуйте -t, щоб запустити тест з точки зору заданого гравця, а не з вашої.");
            }
        }
    }
    ...
    private Block traceBlock(CommandSender sender, Player target, boolean fromTarget) throws CommandException {
        ...
        throw new CommandException("Наразі не розглядається блок, який є достатньо близьким.");
    }
    ...
    private Entity traceEntity(CommandSender sender, Player target, boolean fromTarget) throws CommandException {
        ...
        throw new CommandException("Наразі не розглядається сутність, яка знаходиться достатньо близько.");
    }
    ...
    public void testBreak(Actor sender, LocalPlayer target, boolean fromTarget, boolean stackTraceMode) throws CommandException {
        ...
        sender.print(TextComponent.of("Тестування BLOCK BREAK на ", TextColor.AQUA).append(TextComponent.of(block.toString(), TextColor.DARK_AQUA)));
    ...
    public void testPlace(Actor sender, LocalPlayer target, boolean fromTarget, boolean stackTraceMode) throws CommandException {
        ...
        Block block = traceBlock(bukkitSender, bukkitTarget, fromTarget);
        sender.print(TextComponent.of("Тестування BLOCK PLACE на ", TextColor.AQUA).append(TextComponent.of(block.toString(), TextColor.DARK_AQUA)));
    ...
    public void testInteract(Actor sender, LocalPlayer target, boolean fromTarget, boolean stackTraceMode) throws CommandException {
        ...
        Block block = traceBlock(bukkitSender, bukkitTarget, fromTarget);
        sender.print(TextComponent.of("Тестування BLOCK INTERACT на ", TextColor.AQUA).append(TextComponent.of(block.toString(), TextColor.DARK_AQUA)));
    ...
    public void testDamage(Actor sender, LocalPlayer target, boolean fromTarget, boolean stackTraceMode) throws CommandException {
        ...
        sender.print(TextComponent.of("Тестування ENTITY DAMAGE на ", TextColor.AQUA).append(TextComponent.of(entity.toString(), TextColor.DARK_AQUA)));
