...
public class BukkitStringMatcher implements StringMatcher {
    ...
    public World matchWorld(Actor sender, String filter) throws CommandException {
        ...
        if (filter.charAt(0) == '#') {
            ...
            if (filter.equalsIgnoreCase("#main")) {
                ...
                throw new CommandException("Не знайдено звичайного світу.");
                ...
                throw new CommandException("Не знайдено нижнього світу.");
                ...
                throw new CommandException("Не знайдено енд світу.");
                ...
                if (parts.length == 1) {
                    throw new CommandException("Очікувався аргумент для #player.");
                }

                return matchPlayers(sender, parts[1]).iterator().next().getWorld();
            } else {
                throw new CommandException("Недійсний ідентифікатор '" + filter + "'.");
            }
        }
        ...
        throw new CommandException("Не знайдено світу з такою назвою.");
    }
    ...
    public Iterable<? extends LocalPlayer> matchPlayers(Actor source, String filter) throws CommandException {
        if (Bukkit.getServer().getOnlinePlayers().isEmpty()) {
            throw new CommandException("Жодна особа не відповідає запиту.");
        }
        ...
        if (filter.charAt(0) == '#') {
            ...
            if (filter.equalsIgnoreCase("#world")) {
                ...
                throw new CommandException("Недійсна група '" + filter + "'.");
            }
        }
