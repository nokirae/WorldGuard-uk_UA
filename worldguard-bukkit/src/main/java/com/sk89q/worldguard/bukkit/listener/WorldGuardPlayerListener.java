        ...
        if (session != null) {
            ...
            if (handler != null && wcfg.useRegions && !WorldGuard.getInstance().getPlatform().getSessionManager().hasBypass(localPlayer,
                ...
                if (handler.getOriginalGameMode() != null && expected != null && expected != BukkitAdapter.adapt(event.getNewGameMode())) {
                    log.info("Зміну ігрового режиму " + player.getName() + " було заблоковано через прапор GAMEMODE регіону");
    ...
    public void onPlayerJoin(PlayerJoinEvent event) {
        ...
        if (cfg.activityHaltToggle) {
            player.sendMessage(ChatColor.YELLOW
                    + "Інтенсивна активність сервера була ПРИПИНЕНА.");
            ...
            if (removed > 10) {
                log.info("Зупинка: " + removed + " сутностей (>10) автоматично видалено в "
                        + player.getWorld());
            }
        }
        ...
        if (wcfg.fireSpreadDisableToggle) {
            player.sendMessage(ChatColor.YELLOW
                    + "Поширення вогню в даний час глобально відключено для цього світу.");
        }
        ...
        if (hostKey != null) {
            ...
            if (!hostname.equals(hostKey)
                ...
                event.disallow(PlayerLoginEvent.Result.KICK_OTHER,
                        "Ви не підключилися за допомогою дійсного ключа хоста!");
                log.warning("Перевірка ключа хоста WorldGuard: " +
                        player.getName() + " долучається з '" + hostname +
                        "' але '" + hostKey + "' був очікуваним. Вигнано!");
                return;
            }
        }
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (wcfg.removeInfiniteStacks
            ...
            if (heldItem != null && heldItem.getAmount() < 0) {
                ...
                player.sendMessage(ChatColor.RED + "Нескінченний стак видалено.");
            }
        }
    }
    ...
    private void handleBlockRightClick(PlayerInteractEvent event) {
        ...
        if (Materials.isInventoryBlock(type)
            ...
            for (int slot = 0; slot < 40; slot++) {
                ...
                if (heldItem != null && heldItem.getAmount() < 0) {
                    player.getInventory().setItem(slot, null);
                    player.sendMessage(ChatColor.RED + "Нескінченний стак в слоті #" + slot + " видалено.");
                }
            }
        }
        ...
        if (wcfg.useRegions) {
            ...
            if (item != null && item.getType().getKey().toString().equals(wcfg.regionWand) && getPlugin().hasPermission(player, "worldguard.region.wand")) {
                ...
                if (set.size() > 0) {
                    player.sendMessage(ChatColor.YELLOW + "Ви можете будувати? " + (set.testState(localPlayer, Flags.BUILD) ? "Так" : "Ні"));
                    ...
                    localPlayer.print("Присутні регіони: " + str);
                } else {
                    localPlayer.print("WorldGuard: Тут немає визначених регіонів!");
                }
    ...
    public void onPlayerCommandPreprocess(PlayerCommandPreprocessEvent event) {
        ...
        if (wcfg.useRegions && !WorldGuard.getInstance().getPlatform().getSessionManager().hasBypass(localPlayer, localPlayer.getWorld())) {
            ...
            if (!test.apply(event.getMessage())) {
                ...
                RegionProtectionListener.formatAndSendDenyMessage("використання " + event.getMessage(), localPlayer, message);
                event.setCancelled(true);
                return;
            }
        }
        ...
        if (cfg.blockInGameOp) {
            if (opPattern.matcher(event.getMessage()).matches()) {
                player.sendMessage(ChatColor.RED + "/op та /deop можуть бути використані тільки в консолі (як встановлено WG налаштуваннями).");
                event.setCancelled(true);
                return;
            }
        }
